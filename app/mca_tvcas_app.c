...
static void mca_tvcas_app_message_callback(MCA_CAS_TYPE_t enCASType, MCA_EVENT_TYPE_t enEvtType, MCA_VOID *pData)
{
    MCA_MESSAGE_CODE_t enCode;

    if (MCA_CAS_TVCAS != enCASType)
    {
...
static void mca_tvcas_app_smc_info_callback(MCA_CAS_TYPE_t enCASType, MCA_EVENT_TYPE_t enEvtType, MCA_VOID *pData)
{
...
static void mca_tvcas_app_sub_callback(MCA_CAS_TYPE_t enCASType, MCA_EVENT_TYPE_t enEvtType, MCA_VOID *pData)
{
    MCA_U32 i;
...
static MCA_VOID mca_tvcas_app_unpair_callback(MCA_CAS_TYPE_t enCASType, MCA_EVENT_TYPE_t enEvtType, MCA_VOID *pData)
{
    char     szError[20];
...
{
    if (MCA_CAS_TVCAS != MCA_GetCASType())
    {
        MCA_APP_DBG("CAS Type does not match!");
        return MCA_FAILURE;
    }

    MCA_RegisterEvent(MCA_EVENT_MESSAGE_CODE,   mca_tvcas_app_message_callback);
    MCA_RegisterEvent(MCA_EVENT_SMC_INFO,       mca_tvcas_app_smc_info_callback);
    MCA_RegisterEvent(MCA_EVENT_SUB_INFO,       mca_tvcas_app_sub_callback);
    MCA_RegisterEvent(MCA_EVENT_UNPAIR,         mca_tvcas_app_unpair_callback);
