...
MCA_S32 MCA_OS_TaskCreate(MCA_HANDLE *const pTaskHandle, const MCA_CHAR *szName, \
                          ...
                          MCA_VOID *pStack, MCA_U32 u32StackSize, MCA_TASK_PRIORITY_E enPri)
{
    int32_t  s32GxRet;
    ...
    {
        GX_OS_ERR("Bad Parameter: pTaskHandle = 0x%x, szName = 0x%x, task_func = 0x%x, enPri = %d!\n",   \
                                (unsigned int)pTaskHandle, szName,  \
                                (unsigned int)task_func, (unsigned int)enPri);
        return MCA_FAILURE;
    }

    s32GxRet = GxCore_ThreadCreate(szName, &hGxThreadID, (void *)task_func, pParam, \
                                        1024*10, GXOS_DEFAULT_PRIORITY);
...
MCA_S32 MCA_OS_QueueCreate(MCA_HANDLE *const pHandle, const MCA_CHAR *szName, const MCA_U32 u32MaxMsgCout)
{
    ...
    {
        GX_OS_ERR("Bad Parameter: pHandle = 0x%x, szName = 0x%x!\n", (unsigned int)pHandle, szName);
        return MCA_FAILURE;
    }
...
MCA_S32 MCA_OS_QueueSend(const MCA_HANDLE handle, MCA_VOID *const pMsg, const MCA_U32 u32MsgSize)
{
    ...
    if (NULL == pMsg)
    {
        GX_OS_ERR("Bad Parameter: Pointer is NULL!\n");
        return MCA_FAILURE;
    }
...
MCA_S32 MCA_OS_QueueReceiveTimeout(const MCA_HANDLE handle, MCA_VOID *const pMsg, \
                                   const MCA_U32 u32MsgSize, const MCA_U32 u32Ms)
{
    int32_t  s32GxRet;
    uint32_t u32Copied;
...
MCA_S32 MCA_OS_SemCreate(MCA_HANDLE *const pSemHandle, const MCA_CHAR *szName, const MCA_U32 u32InitCount)
{
    ...
    {
        GX_OS_ERR("Bad Parameter: pSemHandle is 0x%x, szName = 0x%x!\n", \
...
MCA_S32 MCA_OS_SemLock(const MCA_HANDLE handle)
{
    ...
    if (hGxSem < 0)
    {
        GX_OS_ERR("Bad Parameter: hGxSem(%d) < 0!\n", hGxSem);
        return MCA_FAILURE;
    }
