...
MCA_VOID *mca_malloc(MCA_U32 u32Len)
{
    ...
    if (0 == u32Len)
    {
        MS_MEM_ERR("Bad Parameter: Length is ZERO!\n");
        return NULL;
    }    
...
MCA_VOID  mca_free(MCA_VOID *pMem)
{
    ...
    if (NULL == pMem)
    {
        MS_MEM_ERR("Bad Parameter: Pointer is NULL!\n");
        return;
    }
...
MCA_VOID *mca_memcpy(MCA_VOID *pDest, const MCA_VOID *pSrc, MCA_U32 len)
{
    ...
    {
        MS_MEM_ERR("Bad Parameter: pDest is 0x%x, pSrc is 0x%x, len is 0x%x(%d)!\n",   \
    ...
    {
        MS_MEM_ERR("Bad Parameter: The memory areas must not overlap, pDest = 0x%x, pSrc = 0x%x, len = 0x%x\n", \
                                                                                            pDest, pSrc, len);
        return NULL;
    }
...
MCA_VOID *mca_memset(MCA_VOID *pMem, MCA_U8 ch, MCA_U32 len)
{
    if ((NULL == pMem) || (0 == len))
    {
        MS_MEM_ERR("Bad Parameter: pMem = 0x%x, len = 0x%x(%d)!\n", (unsigned int)pMem, len, len);
        return NULL;
    }
...
MCA_S32 mca_memcmp(const MCA_VOID *pMem1, const MCA_VOID *pMem2, MCA_U32 len)
{
    ...
    {
        MS_MEM_ERR("Bad Parameter: pMem1 = 0x%x, pMem2 = 0x%x, len = 0x%x(%d)!\n", \
